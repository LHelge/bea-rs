...
3. **Validate understanding**: Use `AskUserQuestion` again to confirm your understanding of scope and present key design decisions (e.g., "Should this live in `service.rs` or a new module?", "Does the MCP surface need a new tool?", "Should the CLI expose this behind a flag?"). Resolve ambiguity before proceeding.
